...
def run_training(sub_id: int, run_seed: int, run_options):
    """
        ...
        for brain_name in meta_curriculum.brains_to_curriculums.keys():
            if brain_name not in env.external_brain_names:
                raise MetaCurriculumError('''
                  One of the curricula defined in ''' + curriculum_folder + ''' 
                  does not have a corresponding Brain. Check that the 
                  curriculum file has the same name as the Brain whose curriculum
                  it defines.''')
